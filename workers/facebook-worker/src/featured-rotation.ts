    ...
    const eligibleBusinesses = await db.prepare(`
      ...
      ORDER BY 
        CASE WHEN ftm.tier_level = 'premium' THEN 0 
             ...
             ELSE 2 END,
        CASE WHEN (b.facebook_page_id <> '' AND b.facebook_page_id NOT GLOB '*[^0-9]*') OR TRIM(COALESCE(b.facebook_url, '')) <> '' THEN 0 ELSE 1 END,
...
export async function getFeaturedTierMembers(env: any): Promise<any[]> {
  ...
  const result = await db.prepare(`
    SELECT 
      ...
      ftm.tier_start,
      ftm.tier_end,
      CASE WHEN (b.facebook_page_id <> '' AND b.facebook_page_id NOT GLOB '*[^0-9]*') OR TRIM(COALESCE(b.facebook_url, '')) <> '' THEN 1 ELSE 0 END as has_facebook
