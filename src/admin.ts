 ...
 * These functions help authorized Kiamichi staff manage the directory.
 */
...
export async function handleAdminPage(request: Request, env: Env): Promise<Response> {
  ...
  // Check the application-level admin session created after GitHub authorization.
  const sessionId = getSessionId(request);
  const session = await verifyAdminSession(sessionId, db);

  if (!session) {
    // Cloudflare Access is an outer gate; GitHub remains the application identity check.
    ...
    <div class="min-h-screen flex items-center justify-center">
        <div class="bg-white rounded-xl shadow-xl p-8 w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <p class="text-gray-600 mt-2">Kiamichi staff access</p>
            </div>
